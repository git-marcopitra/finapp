...
import { Modal, View, Text, Alert, SafeAreaView, StyleSheet } from "react-native";
...
const AddIncomeModal: FC<AddModalProps> = ({ open, toggle }) => {
  ...
  const onSubmit = async ({ type, amount }) => {
    try {
      const newUserData: IUser = {
        ...user,
        incomes: [...(user.incomes ?? []), { type, amount: +amount }],
      };
    ...
    <Modal visible={open} transparent={false} animationType={"slide"}>
      <SafeAreaView>
        <View style={addCostStyles.header}>
          <Text style={addCostStyles.headerTitle}>Adicionar novo rendimento</Text>
          <Button
            onPress={toggle}
            variant="secondary"
